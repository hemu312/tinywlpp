...
namespace tinywlpp {
    ...
    public:
            ...
            */
            switch (sym) {
                ...
                case XKB_KEY_F1:
                    ...
                    if (wl_list_length(&this->toplevels) < 2) {
                        break;
                    }
                    toplevel *next_toplevel = wl_container_of(this->toplevels.prev, next_toplevel, link);
                    next_toplevel->focus_toplevel(next_toplevel->xdg_toplevel->base->surface);
                    break;
                default:
                    return false;
            }
        ...
        void server_new_keyboard(struct wlr_input_device *device) {
            ...
            * assumes the defaults (e.g. layout = "us"). */
            ...
            wlr_seat_set_keyboard(this->seat, keyboard->wlr_keyboard);
        ...
        toplevel *desktop_toplevel_at(double lx, double ly, struct wlr_surface **surface, double *sx, double *sy) {
            ...
            * surface in the surface tree of a tinywl_toplevel. */
            struct wlr_scene_node *node = wlr_scene_node_at(&this->scene->tree.node, lx, ly, sx, sy);
        ...
        void reset_cursor_mode() {
            ...
            this->cursor_mode = CURSOR_PASSTHROUGH;
            this->grabbed_toplevel = NULL;
        }

        void process_cursor_move(uint32_t time) {
            /* Move the grabbed toplevel to the new position. */
            toplevel *toplevel = this->grabbed_toplevel;
        ...
        void process_cursor_motion(uint32_t time) {
            ...
            if (this->cursor_mode == CURSOR_MOVE) {
                process_cursor_move(time);
                return;
            } else if (this->cursor_mode == CURSOR_RESIZE) {
            ...
            toplevel *toplevel = desktop_toplevel_at(this->cursor->x, this->cursor->y, &surface, &sx, &sy);
            if (!toplevel) {
    ...
    public:
        ...
        ~output(){
            ...
            wl_list_remove(&this->request_state.link);
    ...
    public:
        ...
        ~toplevel(){
            ...
            wl_list_remove(&this->destroy.link);
            wl_list_remove(&this->request_move.link);
            wl_list_remove(&this->request_resize.link);
            wl_list_remove(&this->request_maximize.link);
            wl_list_remove(&this->request_fullscreen.link);
        }
        
        void begin_interactive(enum cursor_mode mode, uint32_t edges) {
            /* This function sets up an interactive move or resize operation, where the
            * compositor stops propegating pointer events to clients and instead
            * consumes them itself, to move or resize windows. */
            auto *server = this->server;
            struct wlr_surface *focused_surface = server->seat->pointer_state.focused_surface;
            if (this->xdg_toplevel->base->surface !=
            ...
            if (mode == CURSOR_MOVE) {
                ...
                struct wlr_box geo_box;
                wlr_xdg_surface_get_geometry(this->xdg_toplevel->base, &geo_box);
        ...
        void focus_toplevel(struct wlr_surface *surface) {
            ...
            */
            if (keyboard != NULL) {
                wlr_seat_keyboard_notify_enter(seat, this->xdg_toplevel->base->surface,
    ...
    public:
        ...
        ~keyboard(){
            wl_list_remove(&this->modifiers.link);
            wl_list_remove(&this->key.link);
            wl_list_remove(&this->destroy.link);
            wl_list_remove(&this->link);
        }
    };

    // functions that don't belong to any class
    ...
    void server_cursor_button(struct wl_listener *listener, void *data) {
        ...
        * event. */
        ...
        if (event->state == WLR_BUTTON_RELEASED) {
            ...
            toplevel->focus_toplevel(surface);
        }
    }
    ...
    void xdg_toplevel_map(struct wl_listener *listener, void *data) {
        ...
        toplevel->focus_toplevel(toplevel->xdg_toplevel->base->surface);
    }
    ...
    void xdg_toplevel_request_move(struct wl_listener *listener, void *data) {
        ...
        * client, to prevent the client from requesting this whenever they want. */
        ...
        toplevel->begin_interactive(CURSOR_MOVE, 0);
    }
    ...
    void xdg_toplevel_request_resize(struct wl_listener *listener, void *data) {
        ...
        * client, to prevent the client from requesting this whenever they want. */
        ...
        toplevel->begin_interactive(CURSOR_RESIZE, event->edges);
    }
